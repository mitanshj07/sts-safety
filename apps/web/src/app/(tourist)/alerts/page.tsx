  ...
  return (
    <main className="sts-enter mx-auto flex max-w-lg flex-col gap-4 px-4 py-6">
      <PageHeader
        kicker="Inbox"
        title="Alerts"
        description="Geofence warnings and SOS receipts, including when you are offline."
        className="mb-0"
      />
      {rows.length === 0 ? (
        <EmptyState
          icon={Bell}
          title="All quiet"
          description="No notifications yet. Restricted-zone entries and SOS receipts appear here."
        />
      ) : (
        <ul className="space-y-3">
          {rows.map((n) => (
            <li key={String(n.id)} className="rounded-2xl border border-border/80 bg-card/80 p-4 shadow-sm">
