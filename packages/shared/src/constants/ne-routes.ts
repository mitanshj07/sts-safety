// North-East preset itineraries used by public onboarding, skip-to-app,
// and command-centre corridor checks. Coordinates are [lon, lat].

export type PresetWaypoint = {
  name: string
  lat: number
  lon: number
...
export const DEFAULT_ITINERARY_CORRIDOR_M = 2000
export const DEFAULT_ITINERARY_PRESET_ID = "ghy-shillong"
...
export function defaultPresetRoute(): PresetRoute {
  return routeById(DEFAULT_ITINERARY_PRESET_ID) ?? PRESET_NE_ROUTES[0]!
}

export function itineraryLineString(route: PresetRoute): {
  type: "Feature"
  properties: { id: string; title: string; corridor_m: number }
