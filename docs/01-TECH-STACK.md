...
| Indian KYC fetch | **DigiLocker** partner API (MeitY, free) with in-app **demo** consent when credentials are unset | Sign-up (landing / Tourist tab) starts OAuth before a session exists. After allow we pull eAadhaar XML server-side and open onboarding. No card. Offline fallback is typed Aadhaar / Voter ID / DL. |
