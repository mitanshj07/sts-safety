...
| Indian KYC fetch | **DigiLocker** partner API (MeitY, free) with in-app **demo** consent when credentials are unset | After the tourist allows access we pull eAadhaar XML and issued files server-side. No card. Offline fallback is typed Aadhaar / Voter ID / DL. |
