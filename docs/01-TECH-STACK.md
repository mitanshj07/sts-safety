...
| Indian KYC fetch | **In-app DigiLocker portal** matching the real sign-in → allow → issued-document process | Production uses this flow (no MeitY partner client required). Server parses eAadhaar XML and prefills onboarding. Typed Aadhaar / Voter ID / DL remains the offline fallback. |
