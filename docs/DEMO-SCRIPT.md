...
**Phone:** `/login?tab=tourist` → **Continue with DigiLocker** → Allow and fetch → onboarding opens with eAadhaar / name / DOB prefilled (demo: Priya Sharma `2341 2341 2346`) → emergency contact → **Issue digital ID**. Seeded **Enter as Priya Sharma** skips this and lands on `/home`. Manual Aadhaar is the backup if DigiLocker is denied.
