...
- Issuance policy (onboarding step 1, Zod, and `tourists_kyc_matches_nationality`): **Indian (`IN`) travellers must use Aadhaar**, with Voter ID or driving licence as equivalent Indian KYC. Indians can **Continue with DigiLocker**: after they allow access the server fetches eAadhaar XML and issued documents and prefills the form. **International travellers must use a passport.** Aadhaar numbers are 12 digits (first digit 2–9) with a Verhoeff checksum; passport numbers follow ICAO 9303 shape.
