...
- Issuance policy (onboarding step 1, Zod, and `tourists_kyc_matches_nationality`): **Indian (`IN`) travellers must use Aadhaar**, with Voter ID or driving licence as equivalent Indian KYC. Indians start from **sign-up with DigiLocker** (landing or Tourist login tab): sign in, allow issued documents, then the server parses eAadhaar XML, opens a tourist session if needed, and prefills the form. **International travellers must use a passport.** Aadhaar numbers are 12 digits (first digit 2–9) with a Verhoeff checksum; passport numbers follow ICAO 9303 shape.
